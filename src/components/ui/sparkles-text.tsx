...
interface Sparkle {
  id: string; x: string; y: string; color: string
  delay: number; scale: number; lifespan: number
}
...
const SparkleEl = ({ id, x, y, color, delay, scale }: Sparkle) => (
  <motion.svg
    key={id}
...
export const SparklesText: React.FC<SparklesTextProps> = ({
  text, colors = { first: '#14b8a6', second: '#38bdf8' }, className, sparklesCount = 10,
}) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setSparkles(cur => cur.map(s => s.lifespan <= 0 ? generateStar() : { ...s, lifespan: s.lifespan - 0.1 }))
    }, 100)
